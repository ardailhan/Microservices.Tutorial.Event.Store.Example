...
await eventStoreService.SubscribeToStreamAsync(
    ...
    async (ss, re, ct) =>
    {
        ...
        await Console.Out.WriteAsync("******Balance******");
...
class EventStoreService
{
    ...
    public EventData GenerateEventData(object @event) => new(eventId: Uuid.NewUuid(),type: @event.GetType().Name, data: JsonSerializer.SerializeToUtf8Bytes(@event));
